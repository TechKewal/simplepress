...
/*	============================================================================================
	sp_do_MentionsYourLatestTag()

	displays the latest mentions

	parameters:
		name			description								type			default
		----------------------------------------------------------------------------------------
		tagId			unique id to use for div or list		text			spMentionsYourLatestTag
...
function sp_do_MentionsYourLatestTag($args='') {
	#check if forum displayed
	if (sp_abort_display_forum()) return;

	$defs = array('tagId'		=> 'spMentionsYourLatestTag',
				  'tagClass'	=> 'spListTag',
	...
	$a = apply_filters('sph_MentionsYourLatestTag_args', $a);
	extract($a, EXTR_SKIP);
    ...
    $recs = SP()->activity->get('type='.SPACTIVITY_MENTIONED."&uid=SP()->user->thisUser->ID&limit=".$options['latest_number']);
	...
	if (SP()->forum->view->has_postlist()) : while(SP()->forum->view->loop_postlist()) : SP()->forum->view->the_postlist();
		...
		for ($x=0; $x<strlen($itemOrder); $x++) {
			...
			switch (substr($itemOrder, $x, 1)) {
				...
				case 'M':
					# user
					$display_name = SP()->user->thisUser->display_name;
	...
	$out = apply_filters('sph_MentionsYourLatestTag', $out);
...
function sp_do_MentionsYourLatestShortcode($atts) {
	$args = array();
