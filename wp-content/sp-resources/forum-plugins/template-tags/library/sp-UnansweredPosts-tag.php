...
function sp_do_sp_UnansweredPostsTag($args='') {
	...
	$defs = array('tagId'    	=> 'spUnansweredPostsTag',
				  'tagClass' 	=> 'spListTag',
    ...
    $where = SPTOPICS.'.post_count=1';

	# do we have forum ids specified?
	$where.= (!empty($forumIds)) ? ' AND '.SPPOSTS.'.forum_id IN ('.$forumIds.')' : '';
...
function sp_do_UnansweredPostsShortcode($atts) {
    $args = array();
