...
/*
Simple:Press
Topic Push Notifications (Pushbullet) plugin ajax routine for management functions
...
if (!SP()->auths->get('pushbullet')) die();

if (!sp_nonce('subs-pushbullet')) die();
...
if ($action == 'add-sub') {
    ...
    sp_push_notifications_save_subscription($topic, SP()->user->thisUser->ID, false, $sub_action, SPACTIVITY_SUBS_PUSHBULLET_TOPIC);
	die();
...
if ($action == 'del-sub') {
    ...
    $topic = SP()->filters->integer($_GET['topic']);
	$sub_action = substr(strstr($_GET['action'], '-'), 1, strlen($_GET['action']));
    sp_push_notifications_remove_subscription($topic, SP()->user->thisUser->ID, false, $sub_action, SPACTIVITY_SUBS_PUSHBULLET_TOPIC);
	die();
	
}

if ($action == 'update-topic-pushbullet') {
    ...
    if (SP()->activity->get_col('col=item&type='.SPACTIVITY_SUBS_PUSHBULLET_TOPIC.'&uid='.SP()->user->profileUser->ID )) {
		...
		if(isset($_GET['checked']) && isset($_GET['topic'])){
			...
			foreach($_GET['topic'] as $key=>$value){
				...
				sp_push_notifications_remove_subscription(
					...
					false, 
					$sub_action, 
					SPACTIVITY_SUBS_PUSHBULLET_TOPIC
				);
			...
			foreach (SP()->activity->get_col('col=item&type='.SPACTIVITY_SUBS_PUSHBULLET_TOPIC.'&uid='.SP()->user->profileUser->ID ) as $sub) {
				$sub_action = substr(strstr($_GET['action'], '-'), 1, strlen($_GET['action']));
				sp_push_notifications_remove_subscription(
