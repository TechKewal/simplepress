...
/*
Simple:Press
Profile Manage Pushbullet Form
...
$ajaxURL = htmlspecialchars_decode(wp_nonce_url(SPAJAXURL."subs-pushbullet&targetaction=update-topic-pushbullet&user=$userid", 'subs-pushbullet'));
?>
<script>
...
if (!property_exists(SP()->user->profileUser, 'pushbullet') || empty(SP()->user->profileUser->pushbullet)) {

	SP()->user->profileUser->pushbullet = SP()->activity->get_col('col=item&type='.SPACTIVITY_SUBS_PUSHBULLET_TOPIC.'&uid='.SP()->user->profileUser->ID );
	
}


if (SP()->user->profileUser->pushbullet) {
    ...
    $found = false;
	foreach (SP()->user->profileUser->pushbullet as $sub) {
