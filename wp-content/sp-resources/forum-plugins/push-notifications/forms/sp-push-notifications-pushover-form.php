...
$ajaxURL = htmlspecialchars_decode(wp_nonce_url(SPAJAXURL."subs-pushover&targetaction=update-topic-pushover&user=$userid", 'subs-pushover'));
?>
<script>
	...
	$(document).ready(function($) {
		...
		var defaultAction = $("input[name=formsubmit]").closest('form').attr('action');
...
if (!property_exists(SP()->user->profileUser, 'pushover') || empty(SP()->user->profileUser->pushover)) {

	SP()->user->profileUser->pushover = SP()->activity->get_col('col=item&type='.SPACTIVITY_SUBS_PUSHOVER_TOPIC.'&uid='.SP()->user->profileUser->ID );
	
}


if (SP()->user->profileUser->pushover) {
	...
	foreach (SP()->user->profileUser->pushover as $sub) {
