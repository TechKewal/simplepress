...
/*
Simple:Press
Warnings and Suspensions Plugin Admin Bans Save Routine
...
function sp_warnings_suspensions_do_bans_save() {
	check_admin_referer('forum-adminform_userplugin', 'forum-adminform_userplugin');

    # get the data
    $name = SP()->filters->str($_POST['banuser']);
    $userid = SP()->DB->table(SPMEMBERS, "display_name='$name'", 'user_id');
    $ugid = SP()->filters->integer($_POST['usergroup_id']);

    # validity checks
    if (empty($name) || empty($userid)) return __('No ban added. Please complete all entries.', 'sp-warnings-suspensions');
    ...
    # create db entry
	SP()->DB->execute("INSERT INTO ".SPWARNINGS." (warn_type, user_id, display_name, usergroup, saved_memberships) VALUES (".SPWARNBAN.", $userid, '$name', '$ugname', '$membership_list')");
	...
	return __('User Ban Added', 'sp-warnings-suspensions');
}
