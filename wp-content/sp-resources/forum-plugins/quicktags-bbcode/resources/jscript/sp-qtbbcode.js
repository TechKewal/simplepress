/* ---------------------------------
Simple:Press
QuickTags=BBCODE Editor Plugin Javascript
------------------------------------ */
...
(function(spj, $, undefined) {
	...
	spj.editorInsertContent = function(intro, content) {
		...
		document.addpost.postitem.value += '[POSTQUOTE][b]'+intro+'[/b]\r'+content+s+'[/POSTQUOTE]\r\r';
		document.addpost.postitem.focus();
	};
	...
	spj.editorInsertAttachment = function(file, title, path, item, width, height, twidth, theight) {
		$('#postitem').val($('#postitem').val()+'<img src="'+path+file+'" title="'+title+'" alt="'+title+'" width="'+width+'" height="'+height+'"/>');
	};
