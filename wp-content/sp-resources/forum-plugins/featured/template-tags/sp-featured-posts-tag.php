...
/* 	============================================================================================
	...
	parameters:
		...
		----------------------------------------------------------------------------------------
		tagId			unique id to use for div or list		text			spFeaturedPostsTag
		...
		postIds		    comma delimited list of post id's		optional
						- if included results will be limited to these posts
                        - otherwise the post IDs in featured posts sfmeta will be used
		...
		beforePost		text to display before the post/excerpt	text			'Forum: '
		afterPost		text to display after the post/excerpt	text			'<br />'
...
function sp_do_sp_FeaturedPostsTag($args='') {
	...
	$defs = array('tagId'    	=> 'spFeaturedPostsTag',
				  'tagClass' 	=> 'spListTag',
	...
	$postTip		= (int) $postTip;
	$beforePost     = SP()->displayFilters->title($beforePost);
	$afterPst  	    = SP()->displayFilters->title($afterPost);
	$echo			= (int) $echo;
    ...
    if (empty($postIds)) {
        $posts = SP()->meta->get('featured', 'posts');
        $postIds = implode(',', $posts[0]['meta_value']);
        if (empty($postIds)) return '';
    }
	$where = SPPOSTS.'.post_id IN ('.$postIds.')';
		...
		for ($x=0; $x<strlen($itemOrder); $x++) {
			switch (substr($itemOrder, $x, 1)) {
				...
				case 'P':
					# Post
					$out.= $beforePost;
					$out.= SP()->forum->view->thisListPost->post_content;
					$out.= $afterPost;
					break;
				...
				case 'E':
					# Excerpt
					$out.= $beforePost;
					$out.= SP()->forum->view->thisListPost->post_tip;
					$out.= $afterPost;
					break;
...
function sp_do_FeaturedPostsShortcode($atts) {
    $args = array();
    ...
    if (isset($atts['postids']))        $args['postIds']        = $atts['postids'];
    ...
    if (isset($atts['beforepost']))     $args['beforePost']     = $atts['beforepost'];
    if (isset($atts['afterpost']))      $args['afterPost']      = $atts['afterpost'];
