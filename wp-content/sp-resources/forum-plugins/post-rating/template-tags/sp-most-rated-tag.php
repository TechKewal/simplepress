...
/* 	=====================================================================================
	sp_rating_do_most_rated_posts_tag($limit, $forum, $user, $postdate, $listtags, $forumids)
	...
	parameters:
 	...
 	===================================================================================*/
function sp_rating_do_most_rated_posts_tag($limit=10, $forum=true, $user=true, $postdate=true, $listtags=true, $forumids=0, $count=true, $avg=true) {
		...
		foreach ($sfposts as $sfpost) {
			if (SP()->auths->can_view($sfpost->forum_id, 'topic-title')) {
				...
				if ($user) {
    				...
    				$out.= $poster;
    				if ($count || ($avg && $postratings['ratingsstyle'] == 2)) {
                    	$postratings = SP()->options->get('postratings');
    				    $out.= ' (';
                        if ($count) {
                            $out.= __('rated', 'sp-rating').' '.$sfpost->vote_count.' '.__('times', 'sp-rating');
                            if ($avg && $postratings['ratingsstyle'] == 2) $out.= ' '.__('with', 'sp-rating').' ';
                        }
    				    if ($avg && $postratings['ratingsstyle'] == 2) $out.= sprintf(__('average rating of %.2f', 'sp-rating'), ($sfpost->ratings_sum / $sfpost->vote_count));
    				    $out.= ')';
                    }
    			}
    			if ($listtags) $out.= '</li>';
    ...
    $out = apply_filters('sph_most_rated_post_tag', $out, $sfpost);
	echo $out;
}
